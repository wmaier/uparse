...
public class CompoundSwapTransitionifier extends SwapTransitionifier {

	public CompoundSwapTransitionifier(UparseOptions opts) {
		super(opts);
	}
	...
	public void getTransitions(List<Tree> preTerminals, List<DiscoTransition> result, List<Tree> stack) throws ParserException {
		while (preTerminals.size() > 0 || stack.size() > 0) {
			...
			if (preTerminals.size() > 0) {
				...
				while (!nextLc.equals(preTerminals.get(0))) {
					result.add(new ShiftTransition());
					swapb.add(preTerminals.get(0));
					stack.add(preTerminals.get(0));
					preTerminals.remove(0);
				}
				result.add(new ShiftTransition());
				stack.add(preTerminals.get(0));
				preTerminals.remove(0);
				CompoundSwapTransition cswap = new CompoundSwapTransition();
				if (swapb.size() > 0)
					result.add(cswap);
				while (swapb.size() > 0) {
					cswap.grow();
					stack.remove(stack.size() - 2);
					preTerminals.add(0,swapb.remove(swapb.size() - 1));
				}
			}
		}
	}
